...
# this script downloads outbreaks PDF files
# from the Integrated Disease Surveillance Programme (IDSP) for 2010 to 2018
...
for row in relevant_rows:
    ...
    for file_number in range(0, number_of_files):
        ...
        # the links to some files seem to be broken, so these files need to be downloaded manually
            ...
            r = requests.get(
                "https://www.idsp.nic.in/WriteReadData/DOB2011/52nd_wk11_u.pdf", verify=False
            )
            with open(f"{year}/{file_name}", "wb") as f:
                f.write(r.content)
        elif file_name == "9th_2013.pdf":
            r = requests.get(
                "https://www.idsp.nic.in/WriteReadData/DOB2013/9th_wk13.pdf", verify=False
            )
            with open(f"{year}/{file_name}", "wb") as f:
                f.write(r.content)
        elif file_name == "15th_2016.pdf":
            r = requests.get(
                "https://www.idsp.nic.in/WriteReadData/l892s/25655158311465983344.pdf", verify=False
            )
            with open(f"{year}/{file_name}", "wb") as f:
                f.write(r.content)
        elif file_name == "4th_2017.pdf":
            r = requests.get(
                "https://www.idsp.nic.in/WriteReadData/l892s/15366628751488435411.pdf", verify=False
            )
            with open(f"{year}/{file_name}", "wb") as f:
                f.write(r.content)
