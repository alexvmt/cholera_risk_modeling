...
# this script extracts and cleans cholera outbreaks
# from the downloaded outbreaks PDF files
...
def extract_and_clean_cholera_outbreaks(file):
    ...
    for i in range(0, len(tables_raw)):
        ...
        table_raw.iloc[:, 0] = table_raw.apply(
            lambda x: (
                "district missing"
                if (x.iloc[0] == "") & ("cholera" in x.iloc[1].lower())
                else x.iloc[0]
            ),
            axis=1,
        )
        ...
        if year <= 2011:
            table_raw.iloc[:, 3] = table_raw.iloc[:, 2].apply(
                lambda x: x.split("/")[1] if len(x.split("/")) > 1 else ""
            )
            table_raw.iloc[:, 2] = table_raw.iloc[:, 2].apply(
                lambda x: x.split("/")[0] if len(x.split("/")) > 1 else x
            )

        # keep only rows containing cholera outbreaks
        table_raw = table_raw[
            table_raw.apply(lambda x: x.str.contains("cholera", case=False).any(), axis=1)
        ]
        ...
        table_raw.iloc[:, 1] = table_raw.iloc[:, 1].apply(
            lambda x: x.replace("i", "").replace("x", "").replace("v", "").replace(".", "").strip()
        )
...
for year in years:
    ...
    for file in files:
        ...
        cholera_outbreaks = pd.concat(
            [cholera_outbreaks, extracted_cleaned_cholera_outbreaks], ignore_index=True
        )
